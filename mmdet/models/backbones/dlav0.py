...
class DLAv0(nn.Module):
    def __init__(self, base_name,
                 pretrained=True, down_ratio=4):
        super(DLAv0, self).__init__()
    ...
    def forward(self, x):
        ...
        # torch.cuda.synchronize()
        # temp = time()
        # time_dict['base_time'] = torch.tensor(temp - end)
        # end = temp
        # breakpoint()
        x = self.dla_up(x[self.first_level:])
        # torch.cuda.synchronize()
        # temp = time()
        # time_dict['dla_up_time'] = torch.tensor(temp - end)
        # print('project - {}'.format(timer))
        # print('upsample - {}'.format(timer1))
        return x, time_dict
        # x = self.fc(x)
...
def dla34up(classes, pretrained_base=None, **kwargs):
    model = DLAv0('dla34', classes, pretrained_base=pretrained_base, **kwargs)
    return model


def dla60up(classes, pretrained_base=None, **kwargs):
    model = DLAv0('dla60', classes, pretrained_base=pretrained_base, **kwargs)
    return model


def dla102up(classes, pretrained_base=None, **kwargs):
    model = DLAv0('dla102', classes,
                   pretrained_base=pretrained_base, **kwargs)
    return model


def dla169up(classes, pretrained_base=None, **kwargs):
    model = DLAv0('dla169', classes,
                   pretrained_base=pretrained_base, **kwargs)
    return model
'''

# def get_pose_net(num_layers, heads, add_conv=256, down_ratio=4):
#   model = DLAv0('dla{}'.format(num_layers), heads,
#                  pretrained=True,
